...
def process_frame(frame):
    ...
    section_width = int(width / 6)  # Calculate the width of each section
    ...
    upper_black = np.array([40,255,255], dtype=np.uint8)  # 占썅간占쏙옙 占쌈계값 占쌩곤옙占싹울옙 占쏙옙占쏙옙占쏙옙 占쏙옙占쏙옙
    ...
    if len(contours) >= 1:
        ...
        if M["m00"] != 0:
            ...
            center_lines = [section_width * (2 * i + 1) // 2 for i in range(6)]  # Calculate center lines for each section
            if 0 <= cx < section_width:
                return "LEFT"
            elif section_width <= cx < 5 * section_width:
                return "FORWARD"
            elif 5 * section_width <= cx < width:
                return "RIGHT"
