    ...
    def run(self):
        while not self.stopped:
            ...
            if result:
                print(result)
                if result == "LEFT":
                    threading.Timer(0.01, agv.counterclockwise_rotation(2)).start()
                elif result == "FORWARD":
                    threading.Timer(0.01, agv.go_ahead(3)).start()
                elif result == "RIGHT":
                    threading.Timer(0.01, agv.clockwise_rotation(2)).start()
...
def process_frame(frame):
    ...
    if len(contours) >= 1:
        ...
        if M["m00"] != 0:
            ...
            if 0 <= cx < 2 * section_width:
                return "LEFT"
            elif 2 * section_width <= cx < 3 * section_width:
                return "FORWARD"
            elif 3 * section_width <= cx < width:
                return "RIGHT"
