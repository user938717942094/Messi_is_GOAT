...
def process_frame(frame):
    ...
    lower_grey = np.array([40,11,0], dtype=np.uint8)
    upper_grey = np.array([90,161,91], dtype=np.uint8)
    ...
    contours_purple, _ = cv2.findContours(grey_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def perform_route_action():
    print("grey detected. Performing route action...")
    agv.stop()
    time.sleep(1)
